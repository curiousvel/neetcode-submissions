    ...
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for right in range(n):
            ...
            while s[right] in sett:
                sett.remove(s[left])  # <-- Note: You had s[right] here by typo, it should be s[left] to drop the front element!
                left += 1

            # Add the new character and calculate the max unique window
            sett.add(s[right])
            length = right - left + 1
